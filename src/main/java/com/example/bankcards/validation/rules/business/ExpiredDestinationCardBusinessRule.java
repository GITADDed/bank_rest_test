...
@Component
public class ExpiredDestinationCardBusinessRule implements BusinessRule<TransferData> {

    @Override
    public Optional<TransferViolation> applyRule(TransferData data) {
        if (CardUtils.isExpired(data.to(), LocalDate.now())) {

            return Optional.of(new TransferViolation(new Violation("toCardId",
                    "Destination card is expired"), Optional.of(data.to().getId())));
        }
